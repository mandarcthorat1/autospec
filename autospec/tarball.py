...
def process_go_dependency(url, target):
    ...
    list_file = os.path.join(target, "list")
    if not os.path.exists(list_file):
        download.do_curl(url,
                         list_file,
                         is_fatal=True)
    sha1 = get_sha1sum(list_file)
    write_upstream(sha1, "list")
...
def process(url_arg, name_arg, ver_arg, target, archives_arg, filemanager):
    ...
    # '-' aren't allowed in the version string for spec files
    # rpmbuild will fail so replace them with '.'
    if '-' in version:
    ...
    tar_path = check_or_get_file(url, os.path.basename(url))
